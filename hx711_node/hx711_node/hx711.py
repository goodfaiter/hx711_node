...
# __version__ = '1.2.0'
# Update from this by Valentin Yuryev for hx711_node package on Rasp Pi 5
...
class HX711(object):
    ...
    def _validate_measure_count(self, times):
        ...
        if not self.min_measures <= times <= self.max_measures:
            raise ParameterValidationError("{times} is not within the min/max range defined in the class".format(times=times))
